...
description: >-
  A deep dive into solving RedBeat's Redis Sentinel username authentication
  limitation through a configuration workaround.
---
...
<Highlight color="red">
  Modern Redis deployments use ACL (Access Control Lists) that require both
  username and password for authentication.
</Highlight>
However, when I tried to start RedBeat with Sentinel, I kept getting
...
Here are some test cases for the newly added MDX components:

### Highlight Component

This is a <Highlight color="green">green highlight to emphasize an important point.
You can also use a custom color if needed.</Highlight>

### Kbd Component

To save your work, press <Kbd>Cmd</Kbd> + <Kbd>S</Kbd> (or <Kbd>Ctrl</Kbd> + <Kbd>S</Kbd> on Windows).
To open the command palette, use <Kbd>Shift</Kbd> + <Kbd>Cmd</Kbd> + <Kbd>P</Kbd>.

### Accordion Component

<Accordion title="Click to reveal more details about Redis ACL">
  Redis Access Control Lists (ACL) provide fine-grained control over user
  permissions, allowing administrators to define which commands and keys a user
  can access. This significantly enhances the security posture of a Redis
  deployment compared to traditional password-only authentication.
</Accordion>

<Accordion title="Expand for a deeper dive into Celery Beat schedulers">
  Celery Beat is a scheduler that kicks off tasks periodically. While the
  default scheduler stores tasks in the local filesystem, RedBeat stores them in
  Redis, enabling dynamic task management without needing to restart the Beat
  process.
</Accordion>

### Tooltip Component

The <Tooltip content="An asynchronous task queue/job queue based on distributed message passing.">Celery</Tooltip> framework is widely used for distributed task processing.
This workaround involves <Tooltip content="Bypassing a security or functional restriction through unconventional means.">piggybacking</Tooltip> on an existing configuration.
