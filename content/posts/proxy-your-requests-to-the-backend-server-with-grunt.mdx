...
published: 2018-05-15T17:14:13+05:30
status: "published"
---
...
- **https**: If your backend server is an https endpoint, then set this value to `true`.
- **rewrite**: This option allows rewriting of URL when proxying. What this means is that when trying to proxy [http://localhost:8080/api](http://localhost:8080/api) to the backend server, the URL will be rewritten as [http://localhost:8000/backend](http://localhost:8000/backend). The object's key serves as the regex used in the replacement operation, and the object's value is the context of your backend server's service.
