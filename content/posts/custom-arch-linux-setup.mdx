...
Dynamic menus give the same kind of functionality most people are used to. So it was my preferred choice. They can be used to generate full, complex menus on the fly. One of the most popular application for generating dynamic menus is `obmenu-generator`. Though it is not officially connected to Openbox, it is widely used.

In order to have `obmenu-generator` make a menu on demand, the `menu.xml` file should contain the following code as the only entry.
