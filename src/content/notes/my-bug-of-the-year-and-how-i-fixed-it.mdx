...
Since I was not seeing any jaeger spans from the Flask framework side, I suspected that the requests were not reaching the Flask server at all and were timing out (but not dropped) at the WSGI server layer itself. When the client reached its timeout (10s), Go cancels the context and closes the socket by calling the `close()` syscall. While I didn't debug the TCP layer, I suspect the client sent <MDXTooltip content="TCP Reset packet - signals an abrupt connection termination">RST packet</MDXTooltip> causing an abrupt close. When the server received the RST packet, any reads on the socket will result in `connection reset by peer` and writes will result in <MDXTooltip content="Error that occurs when trying to write to a closed socket">Broken pipe</MDXTooltip> error. Gevent WSGI server catches these errors, logs them and then discards the response.
