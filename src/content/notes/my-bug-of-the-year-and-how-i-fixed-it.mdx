...
I recently worked on a Flask based microservice at work. While everything worked normally for first few weeks, a few heavy traffic days brought in front a critical bug I had introduced while coding. This post is a detailed analysis of what went wrong and how I debugged and fixed the issue.

## The Background

I have been working with distributed systems primarily written in Python with Gevent for concurrency for the last several years. However, I never got a chance to actually write a service that uses Gevent. Most of the services I have worked with were implemented by other teams and were in production for years before I started working on them. So, I never really got to experience writing a Gevent-based Python microservice from scratch. Recently, I got an opportunity to do just that. I was tasked with writing a new microservice that was supposed to act as an email notification service for our platform. This service was replacing another service that belonged to a different product which is getting deprecated.

## Service Implementation

While I initially proposed using FastAPI for the new service, the team decided to go with Flask since most of our existing services were already using Flask. So, I started working on a new Flask-based microservice that exposed REST endpoints for the internal clients. Clients would call these endpoints with the alert payload, and the service would process the alerts and send email notifications by calling another downstream service over the network within same K8s cluster.
...
- Gevent-based WSGI server as the Application server to handle HTTP requests
...
Since I was not seeing any jaeger spans from the Flask framework side, I suspected that the requests were not reaching the Flask server at all and were timing out (but not dropped) at the WSGI server layer itself. When the client reached its timeout (10s), Go cancels the context and closes the socket by calling the `close()` syscall. While I didn't debug the TCP layer, I suspect the client sent RST packet causing an abrupt close. When the server receives a RST packet, any reads on the socket will result in `connection reset by peer` and writes will result in `Broken pipe` error. Gevent WSGI server catches these errors, logs them and then discards the response.

Here are two possible scenarios that were in picture during this issue -

1. WSGI server read the entire request but Flask hasn't started processing
   ...
   - Work is done, but client never knows = ZOMBIE REQUEST
2. WSGI server read the request and Flask started processing
...
So, the high memory usage answered what was happening. My hypothesis was clear now -

- The clients were sending a high number of requests
- Gevent WSGI server was accepting all these requests and spawning Greenlets to keep the requests in the memory.
- Flask was also processing a high number of requests but it was still not enough to process all the Greenlets.
- Since some of the requests were timing out at the server layer, client kept retrying the requests, adding to the load on the server.

But, why was the Flask framework not able to process all the requests?

## A tale of Python's single threaded nature, Gevent and monkey patching

Python is fundamentally single threaded due to [GIL](https://realpython.com/python-gil/). Flask is no different. When a `socket.recv()` or `requests.get()` is executed, the entire process just blocks and does nothing until this I/O completes. This is particularly bad for web applications because the process can't handle more than one request at a time.
...
Here's where I messed up. I forgot to replace Python's standard blocking I/O with Gevent-compatible versions.
...
The above code replaces Python's standard library at runtime. After it runs:
...
- BLOCKS on `socket.send()` (using `stdlib` socket!)
...
| --------------------- | ----------------- | ------------ | ------------------ |
| P95 Latency           | ~8-10s            | ~500ms       | 8x faster          |
| Request Timeout Rate  | ~30%              | < 0.1%       | 300x reduction     |
| Memory Usage          | 85% (growing)     | 45% (stable) | Stabilized         |
| Client Retry Attempts | 15-17 per request | 0            | Eliminated         |
| Email Service Load    | 700 req/min       | 30 req/min   | Back to normal     |
| Requests processed    | 2000 req/sec      | 50 req/sec   | no retries anymore |
